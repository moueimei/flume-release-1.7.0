...
public class SyslogParser {
  ...
  protected long parseRfc5424Date(String msg) {
    ...
    final String timestampPrefix = msg.substring(curPos, RFC5424_PREFIX_LEN);

    try {
      ts = timestampCache.get(timestampPrefix, new Callable<Long>() {
        @Override
        public Long call() throws Exception {
          return timeParser.parseMillis(timestampPrefix);
        }
      });
